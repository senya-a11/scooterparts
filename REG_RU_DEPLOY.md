...
Яндекс с 2022 года **запрещает** использовать обычный пароль аккаунта для SMTP. Нужно создать отдельный «пароль приложения».

### Как создать пароль приложения для SMTP:

1. Зайти на [id.yandex.ru](https://id.yandex.ru) под аккаунтом `fm.tun@yandex.ru`
2. Слева выбрать **«Безопасность»**
3. Найти раздел **«Пароли приложений»** → нажать **«Создать новый пароль»**
4. Тип приложения — **«Почтовый клиент»**, название — `ScooterParts SMTP`
5. Скопировать сгенерированный пароль (показывается один раз!)
6. В файле `.env` на VPS заменить `SMTP_PASS` на этот пароль:

```bash
nano /app/scooterparts/.env
# Изменить строку:
# SMTP_PASS=<вставить пароль приложения>
```

7. Перезапустить без пересборки образа:
```bash
cd /app/scooterparts
docker compose up -d
```

> Также убедитесь что в настройках почты Яндекс включён SMTP: **Настройки → Все настройки → Почтовые клиенты → Включить IMAP/SMTP**

---

## КАК ОБНОВИТЬ САЙТ (после первого деплоя)

Когда ты внёс изменения в код на своём компьютере и запушил в GitHub — вот как обновить сайт на VPS:

### Стандартное обновление (изменения в коде)

```bash
# 1. Подключиться к серверу (если не подключён)
ssh root@<IP сервера>

# 2. Перейти в папку проекта
cd /app/scooterparts

# 3. Скачать новую версию кода
git pull

# 4. Пересобрать и перезапустить (старые данные сохраняются)
docker compose up -d --build
...
Ожидаемый результат: `scooterparts-app-1  Up` и `scooterparts-postgres-1  Up`

### Обновление только переменных окружения (без изменения кода)

Если менял только `.env` (например, SMTP-пароль):

```bash
cd /app/scooterparts
nano .env             # отредактировать
docker compose up -d  # перезапустить без пересборки (быстро, ~5 сек)
```

### Таблица частых операций

| Задача | Команда |
|---|---|
| Посмотреть логи в реальном времени | `docker compose logs -f app` |
| Посмотреть последние 50 строк лога | `docker compose logs app --tail=50` |
| Перезапустить только приложение | `docker compose restart app` |
| Остановить всё | `docker compose down` |
| Запустить всё | `docker compose up -d` |
| Статус контейнеров | `docker compose ps` |
| Зайти в базу данных | `docker compose exec postgres psql -U scooter_user -d scooter_shop` |
| Обновить код + пересобрать | `git pull && docker compose up -d --build` |
...
# Откатиться к предыдущей версии кода
git log --oneline -5     # найти хеш нужного коммита
git checkout <хеш>       # переключиться
docker compose up -d --build

# Или просто перезапустить текущую версию
